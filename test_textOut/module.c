...
bool init_modem_ok()
{
    ...
    delay5ms(1);
    while (repeat-- > 0 && !c2)
        c2 = md_uart_write_str_ok((unsigned char *)"AT+CMEE=2\r\n", 11, YES);        // Turn on verbose error msg
    repeat = 20;
    delay5ms(3);
    while (repeat-- > 0 && !c3)
...
bool sim_card_ok()
{
    ...
    uint8_t buf[32], temp, i, count = 15;
    bool matches = false;
    
    
    /* Retry <count> number of times before timing out */
    while (count-- != 0)
    { 
    ...
    while (!TMR3IF)
    {
        if (RC1IF)
        {
            ...
            if (temp == 'Y')
            {
                matches = str_matches((uint8_t *)&buf[(uint8_t)(i-12)], (uint8_t *)&cpinResponse[0], sizeof(cpinResponse));
                if (matches)
                {

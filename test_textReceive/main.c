...
void interrupt isr()
{
    if (TMR0IF)
    {
        ...
        {
            TMR0L = ((65536-TMR0_LO_FREQ)%256);
            TMR0H = ((65536-TMR0_LO_FREQ)/256);
        }
        else
        {
            TMR0L = ((65536-TMR0_HI_FREQ)%256);
            TMR0H = ((65536-TMR0_HI_FREQ)/256);
        }
        check_leds();
    }
}
...
void check_leds()
{
    switch (ledPattern) 
    {
        ...
        case STANDBY:
            ledPatternCount++;
            if (ledPatternCount < 5)        B_LED = 0;     
            else                            B_LED = 1;
            
            G_LED = 1;
            if (ledPatternCount>=20)
                ledPatternCount = 0;
            break;
            
        case INTERNET:
            B_LED = (unsigned int)~B_LED;
            G_LED = 1;
            break;
            
        case APN_IP_NOT_SET:
            ledPatternCount++;
            if (ledPatternCount < 5)        { B_LED = 0; G_LED = 1; }
            else if (ledPatternCount <= 9)  { B_LED = 1; G_LED = 0; }
            else ledPatternCount = 0;
            break;
            
        case NETWORK_ERR:
            ledPatternCount++;         
            if (ledPatternCount == 0 || ledPatternCount == 2)       { B_LED = 0; G_LED = 1; }
            else if (ledPatternCount == 4 || ledPatternCount == 6)  { B_LED = 1; G_LED = 0; }
            else if (ledPatternCount % 2 == 1)                      { B_LED = 1; G_LED = 1; }             
            else                                                      ledPatternCount = 0;
            break;
            
        case NETWORK_REGISTER:
            ledPatternCount++;
            if (ledPatternCount < 5)
                G_LED = 0;     
            else
                G_LED = 1;
            B_LED = 1;

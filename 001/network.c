...
bool is_connected()
{
    if (!socket_configured())       // Configure Socket 1 (don't open yet)
        return false;
    if (!md_attached())             // Perform GPRS attach
        return false;
...
bool registration_ok()
{
    ...
    for (uint8_t j = 0; j < 2; j ++)                // Repeat same cmds for +CREG and +CEREG
    {
        if (j == 0)
        {
        ...
        while (!TMR3IF)
        {
            ...
            if (RC1IF)
            {
                ...
                {
                    if (j == 0)
                        matches &= str_matches((uint8_t *)&buf[0], (uint8_t *)&creg_resp[0], sizeof(creg_resp));
                    else if (j == 1)
                        matches &= str_matches((uint8_t *)&buf[0], (uint8_t *)&cereg_resp[0], sizeof(cereg_resp));
            
                    if (j == 0 && buf[i + 1] == '0')
                        if (!md_uart_write_str_ok((unsigned char *)"AT+CREG=1", sizeof(creg), YES)){}
//                            return false;
                    matches &= ((buf[i + 3] == '1') || (buf[i + 3] == '5'));        // Check if device is registered as GSM compact. 
...
bool md_attached()
{
    ...
    uint8_t temp, i, buf[32], cnt = 15;
    bool attached = false;              // False before proven true!
    
    while (!attached && cnt > 0)
    {
        md_uart_write_str_ok((uint8_t *)(&gprs_check[0]), sizeof(gprs_check), NO);
        ...
        while (!TMR3IF)
        {
            ...
            if (RC1IF)
            {
                ...
                if (temp == '\n')
                {
                    if (str_matches((uint8_t *)&buf[0], (uint8_t *)&gprs_resp[0], sizeof(gprs_resp)))
                    {
                        attached = (bool)(buf[i + 1] == '1');
                        if (!attached)
                        {
...
bool set_pdp_ok()
{
    ...
    while (apn != '#')
    {
        apn_buf[i] = ee_read(0, APN_ADDR + i);      // Read entire APN from EEPROM
        i++;
    }

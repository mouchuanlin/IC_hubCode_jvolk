 ...
 * Author: THE MASTER
 *
 * Created on April 25, 2018, 12:49
 */
...
#define TEST_PIN            LATBbits.LATB4
#define TEST_PIN_TRIS       TRISBbits.TRISB4
#define WIFI_INT            LATBbits.LATB5
...
#define _100milliseconds    22500       //orig. 780. Calibrated for Timer0 100ms interval for 8MHz clk
...
uint32_t _1HR = 180000;         //(60min * 60 * 50). Will need to change if you
                                // use 4min WDT
...
bool ready_for_sleep = false;                       // Set true when nothing in queue
bool inButtonMenu = false;                          // Indicate we're in the "main menu" after 5x button presses
bool enableTransmitLEDs = false;
bool alarm_int = false, tamper_int = false;
bool stillTriggered = false, stillTampered = false;
